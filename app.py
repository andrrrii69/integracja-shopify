...
def _pick_tax_symbol(tax_lines) -> str:\
    """\
    Shopify tax_lines[].rate is e.g. 0.23, 0.08...\
    Map to inFakt tax_symbol strings.\
    """\
    if not tax_lines:\
        return '0'\
\
...
\
        services.append(\{\
            'name': f"Wysy\uc0\u322 ka - \{shipping.get('title', 'dostawa')\}",\
            'tax_symbol': _pick_tax_symbol(tax_lines),\
            'quantity': 1,\
        ...
        \})\
\
    # UWAGA: NIE dodajemy osobnej pozycji "Rabat"\
    # Rabaty s\uc0\u261  ju\u380  rozliczone w line_items przez discount_allocations,\
    # a VAT jest brany z tax_lines (Shopify liczy to sp\'f3jnie).\
\
    return services\
\
\
...
\
\
# Tworzenie faktury przy nowym zam\'f3wieniu\
...
\
    ...
    if not invoices:\
        app.logger.warning(f"Brak faktury dla zam\'f3wienia \{oid\}")\
        return False\
\
    invoice_uuid = invoices[0].get('uuid')\
    if not invoice_uuid:\
        app.logger.error(f"Brak UUID faktury w odpowiedzi dla zam\'f3wienia \{oid\}")\
        return False\
\
...
def orders_updated():\
    ...
    if not verify_shopify_webhook(raw, sig):\
        abort(401)\
    order = request.get_json()\
    if create_correction(order, 'Edycja zam\'f3wienia'):\
...
def orders_cancelled():\
    ...
    if not verify_shopify_webhook(raw, sig):\
        abort(401)\
    order = request.get_json()\
    if create_correction(order, 'Anulowanie zam\'f3wienia'):\
